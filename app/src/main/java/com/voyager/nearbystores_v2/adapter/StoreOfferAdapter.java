...
import com.voyager.nearbystores_v2.activities.OffersActivity;
...
public class StoreOfferAdapter {
    ...
    private View prepareView(View layout,Offer offer){
        ...
        detail.setText(offer.getContent().getDescription());

        if(offer.getContent().getPrice()>=0) {
            price.setText(offer.getContent().getPrice() + " " + offer.getContent().getCurrency());
        }else {
