...
public class InboxActivity extends AppCompatActivity implements ListDiscussionAdapter.ClickListener, ListDiscussionAdapter.TouchListener, ViewManager.CustomView, DCMBroadcastReceiver.NetworkStateReceiverListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        list = (RecyclerView) findViewById(R.id.listmessages);
...
//    }
    ...
    public void setupToolbar(){
        ...
        Utils.setFont(this, APP_DESC_VIEW, "Comfortaa_Bold.ttf");
        Utils.setFont(this, APP_TITLE_VIEW, "Comfortaa_Bold.ttf");
        ...
        if(message!=null) {
           ...
           for(int i=0;i<adapter.getItemCount();i++){
               ...
               if(adapter.getItem(i).getSenderUser().getId()==mesageData.getSenderId()){
                   ...
                   Discussion dis = adapter.getItem(i);
                   dis.getMessages().add(0,mesageData);

                   int nbrOfNewMessage = 0;
                   for (int f=0;f<dis.getMessages().size();f++){

                       if(dis.getMessages().get(f).getReceiver_id()==mUser.getId()
                               && (dis.getMessages().get(f).getStatus()<0 )){
                           nbrOfNewMessage++;
                       }
                   }
