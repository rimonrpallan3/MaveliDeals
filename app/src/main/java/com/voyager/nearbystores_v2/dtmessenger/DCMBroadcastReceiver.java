...
import com.voyager.nearbystores_v2.appconfig.Messages;
...
import com.voyager.nearbystores_v2.fragments.SettingsFragment;
...
public class DCMBroadcastReceiver extends BroadcastReceiver {
    ...
    private void runBgApp(Context context, Intent intent){
        ...
        if(ServiceHandler.isNetworkAvailable(context)) {
            ...
            boolean notif_coming_event = PreferenceManager.getDefaultSharedPreferences(context).getBoolean("notif_coming_event", true);
            if(notif_coming_event)
                EventController.notifyIfExist(context);
